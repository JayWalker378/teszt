...
func _drop_healing_pickup() -> void:
	# Create healing pickup at enemy position
	var heal_pickup = RigidBody2D.new()
	var sprite = ColorRect.new()
	var collision = CollisionShape2D.new()
	var shape = RectangleShape2D.new()
	var area = Area2D.new()
	...
	# Setup heal pickup appearance (green square)
	sprite.size = Vector2(16, 16)
	sprite.color = Color.GREEN
	heal_pickup.add_child(sprite)
	
	# Setup RigidBody2D collision for physics
	shape.size = Vector2(16, 16)
	collision.shape = shape
	heal_pickup.add_child(collision)
	
	# Setup Area2D for player detection
	area_shape.size = Vector2(16, 16)
	area_collision.shape = area_shape
	area.add_child(area_collision)
	heal_pickup.add_child(area)
	
	# Add to scene at enemy position with slight upward velocity
	get_parent().add_child(heal_pickup)
	heal_pickup.global_position = global_position
	# Give it a small upward and random horizontal velocity
	heal_pickup.linear_velocity = Vector2(randf_range(-50, 50), -100)
	
	# Connect the signal directly (avoid dynamic script creation)
	area.body_entered.connect(_on_heal_pickup_entered.bind(area))
	
	print("Enemy dropped healing pickup!")

func _on_heal_pickup_entered(area: Area2D, body: Node2D) -> void:
	print("Something entered heal pickup: ", body.name)
	if body.name == "Player":
		print("Player detected!")
		# Try direct healing first
		if body.has_method('heal'):
			print("Player has heal method!")
			body.heal(20)
			print("Called heal(20) on player!")
			# Remove the pickup
			if is_instance_valid(area.get_parent()):
				area.get_parent().queue_free()
				print("Removed pickup!")
		else:
